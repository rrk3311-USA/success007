/**
 * Update eBay Women's Balance Listing
 * 
...
async function updateListing() {
    console.log('📋 Updating Women\'s Balance listing...\n');
    ...
    try {
        ...
        // Update inventory item title
        console.log('\n📝 Updating inventory item title and UPC...');
        const invItem = {
            sku: product.sku,
            product: {
                title: seoTitle,
        ...
        const invResponse = await fetch(`${BASE_URL}/sell/inventory/v1/inventory_item/${product.sku}`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${ACCESS_TOKEN}`,
                'Content-Type': 'application/json',
        ...
        if (invResponse.ok) {
            ...
            if (product.gtin || product.upc) {
                console.log(`   ✅ UPC/GTIN added: ${product.gtin || product.upc}`);
            }
        } else {
            const invError = await invResponse.json().catch(() => ({}));
            console.error('⚠️  Inventory item update had issues:', JSON.stringify(invError, null, 2));
        }
...
async function main() {
    ...
    console.log('eBay Women\'s Balance Listing Update');
    console.log('='.repeat(70));
    console.log('');
    ...
    console.log(`   ✅ SEO-optimized description updated`);
    console.log(`   ✅ Price updated to $${PRICE.toFixed(2)}`);
    console.log('');
    console.log('💡 The listing will update automatically. Check your eBay account!');
    console.log('');
}
