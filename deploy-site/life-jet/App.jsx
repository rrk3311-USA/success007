...
function App() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          ...
          setLocation({ lat: 34.0522, lng: -118.2437 }); // LA default
        }
      );
    }
  ...
  const loadDeals = () => {
    // Simulated deals data - in production, this would come from an API
    const mockDeals = [
      {
        id: 1,
        name: "Thunder Strike Burger",
        restaurant: "Base Nine Diner",
        distance: 0.8,
        price: 8.99,
        originalPrice: 12.99,
        discount: 31,
      ...
      {
        id: 4,
        name: "Sonic Boom Sushi",
        restaurant: "Altitude Sushi",
        distance: 1.5,
        price: 14.99,
        originalPrice: 22.99,
        discount: 35,
      ...
      <div className="main-content">
        ...
        <div className="panel left-panel">
          ...
          {activeTab === 'operations' && (
            <ActiveOperations projects={projects} onToggleMilestone={toggleMilestone} />
          )}
        ...
        <div className="panel center-panel">
          ...
          {activeTab === 'vision' && (
            <VisionMatrix photos={visionPhotos} onPhotosChange={setVisionPhotos} fullWidth />
          )}
          {activeTab === 'operations' && (
            ...
            <div style={{ padding: '32px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
                <div className="info-item">
                  <div className="info-label">Total Deals</div>
                  <div className="info-value">{deals.length}</div>
                </div>
                <div className="info-item">
                  <div className="info-label">Active Operations</div>
                  <div className="info-value">{projects.length}</div>
                </div>
                <div className="info-item">
                  <div className="info-label">Vision Targets</div>
                  <div className="info-value">{visionPhotos.length}</div>
                </div>
                <div className="info-item">
                  <div className="info-label">System Status</div>
                  <div className="info-value" style={{ color: 'var(--emerald)' }}>ONLINE</div>
                </div>
              </div>
            </div>
          )}
